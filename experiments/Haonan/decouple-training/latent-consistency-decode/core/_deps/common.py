"""Shared model-loading + answer-scoring helpers: chat model load, yes/no and letter
scoring, small metrics. Frozen instruct model, eval-only. Defaults to Qwen3-4B.

These helpers read, at the assistant generation point, the model's probability over a small set
...
# Model cache: honour HF_HOME if set, otherwise let transformers use its default cache
# (cache_dir=None). Run Qwen3-4B with thinking DISABLED so it answers directly.
